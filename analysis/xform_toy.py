...
def make_simple_data():
    samples = 86400
    df = pd.DataFrame()
    df['load'] = np.zeros(samples)
    df['load2'] = np.zeros(samples)
    flow(df, 'load', '12:00/PT1H')
    flow(df, 'load2', '14:00/PT1H')
...
with tf.device('/cpu:0'):
    ...
    true_df, loads = make_simple_data()
    make_events(true_df)
